...
- Rate limit: login 429 after limit exceeded
...
async def test_rate_limit_login_returns_429(client: AsyncClient):
    """Login endpoint returns 429 after exceeding rate limit (10/minute)."""
    ...
    try:
        ...
        for i in range(11):
            ...
            if i < 10:
                ...
                assert res.status_code == 429, f"Request 11: expected 429, got {res.status_code}"
                assert "rate limit" in res.json().get("detail", "").lower() or "retry" in res.json().get("detail", "").lower()
    finally:
